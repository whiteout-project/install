...
#
# WSL notes:
#   - This file is fully WSL-compatible as-is.
#   - No changes needed for paths or credentials.
#   - resolve_ubuntu_iso_url uses wget which is available in
#     WSL out of the box (sudo apt install wget if missing).
# ============================================================
...
resolve_ubuntu_iso_url() {
  ...
  # FIX: original used grep -oP with '|' inside [...] which is a
  # literal character match, not alternation. Corrected to use a
  # proper negative lookahead / two-stage filter instead.
  iso_name=$(wget -qO- "${index_url}" \
    | grep -oP "ubuntu-[0-9]+\.[0-9]+\.[0-9]+-live-server-amd64\.iso" \
    | grep -v -E '\.(torrent|zsync)' \
    | sort -V \
    | tail -1)

  if [ -z "${iso_name}" ]; then
    echo "ERROR: Could not detect Ubuntu ${UBUNTU_SERIES} ISO from ${index_url}" >&2
    echo "       Check your internet connection or set UBUNTU_ISO_FILE manually." >&2
    exit 1
  fi
